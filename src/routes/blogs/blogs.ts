...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const where: any = {};
      if (keywordStr) {
        where.blogCategoryName = { contains: keywordStr, mode: "insensitive" };
      }
      if (statusStr) {
      ...
      const totalItems = await prisma.blogCategory.count({ where });
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const where: any = {};
      if (keywordStr) {
        where.postTitle = { contains: keywordStr, mode: "insensitive" };
      }
      if (statusStr) {
        where.isActive = statusStr;
      }
